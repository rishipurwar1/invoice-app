...
const InvoiceCard = ({ data }) => {
    ...
    return (
        <Link to={`/invoice/${data.id}`} className="grid grid-cols-card items-center w-full px-6 py-4 bg-primaryOne mb-4 rounded-lg shadow-md cursor-pointer transition border border-transparent hover:border-purple-500">
            <h2 className="text-white text-xs font-bold"><span className="text-secondaryThree">#</span>{data.id}</h2>
